...
public class Cache {
    ...
    private void createTable() {
        ...
        try (Connection connection = method.getConnection(); PreparedStatement ps = connection.prepareStatement(UtilityFunctions.isMySQL() ? CREATE_TABLE_MYSQL : CREATE_TABLE_H2)) {
            try {
    ...
    public void addInformation(UUID uuid) {
        ...
        try (Connection connection = method.getConnection(); PreparedStatement ps = connection.prepareStatement(ADD_INFORMATION)) {
            try {
                ...
                switch (DEFAULT.valueOf(plugin.getConfigYaml().getString("ON_VOID_TP.SETTINGS.DEFAULT_OPTION"))) {
                    case TELEPORT: {
    ...
    public void updatePlayer(Player player) {
        ...
        data.setTeleportEnabled(getVerify(player.getUniqueId(), "TELEPORT"));
        data.setJumpEnabled(getVerify(player.getUniqueId(), "JUMP"));
        data.setLightningEnabled(getVerify(player.getUniqueId(), "LIGHTNING"));
        data.setParticlesEnabled(getVerify(player.getUniqueId(), "PARTICLES"));
    }
