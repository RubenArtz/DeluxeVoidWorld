...
import artzstudio.dev.deluxevoid.features.sendActionbar;
import artzstudio.dev.deluxevoid.features.sendBossBar;
import artzstudio.dev.deluxevoid.features.sendParticles;
import artzstudio.dev.deluxevoid.features.sendTitles;
import artzstudio.dev.deluxevoid.firework.ColorUtils;
import artzstudio.dev.deluxevoid.firework.FireworkManager;
...
public class UtilityFunctions {
    ...
    private static void getBossbar(Player player) {
        if (plugin.getConfigYaml().getBoolean("ON_VOID_TP.SETTINGS.BOSSBAR.ENABLED")) {
            sendBossBar.sendBoss(player, plugin.getConfigYaml(), "ON_VOID_TP.SETTINGS.BOSSBAR.BAR_COLOR", plugin.getFileTranslations().getString("MESSAGE_BOSSBAR_TP_LOBBY"));
        }
    }

    public static void geCommands(Player player) {
        if (plugin.getConfigYaml().getBoolean("ON_VOID_TP.SETTINGS.COMMANDS.ENABLED")) {
            if (Objects.requireNonNull(plugin.getConfigYaml().getString("ON_VOID_TP.SETTINGS.COMMANDS.TYPE")).contains("CONSOLE")) {
                for (String commands : plugin.getConfigYaml().getStringList("ON_VOID_TP.SETTINGS.COMMANDS.COMMANDS")) {
    ...
    public static void getSound(Player player) {
        if (plugin.getConfigYaml().getBoolean("ON_VOID_TP.SETTINGS.SOUNDS.ENABLED")) {
            UtilityFunctions.executeSound(Objects.requireNonNull(plugin.getConfigYaml().getString("ON_VOID_TP.SETTINGS.SOUNDS.SOUND")), player);
        }
    }

    public static void getTitle(Player player) {
        if (plugin.getConfigYaml().getBoolean("ON_VOID_TP.SETTINGS.TITLES.ENABLED")) {
    ...
    public static void getMessage(Player player) {
        if (plugin.getConfigYaml().getBoolean("ON_VOID_TP.SETTINGS.MESSAGES.ENABLED")) {
    ...
    public static void getActionbar(Player player) {
        if (plugin.getConfigYaml().getBoolean("ON_VOID_TP.SETTINGS.ACTIONBAR.ENABLED")) {
    ...
    public static void getFirework(Player player) {
        if (plugin.getConfigYaml().getBoolean("ON_VOID_TP.SETTINGS.FIREWORK.ENABLED")) {
            ArrayList<Color> colors = new ArrayList<>();
            for (String fireworkColor : plugin.getConfigYaml().getStringList("ON_VOID_TP.SETTINGS.FIREWORK.COLORS")) {
                colors.add(ColorUtils.parseColor(fireworkColor));
            }
            if (!colors.isEmpty()) {
                UtilityFunctions.runTaskLater(25L, () ->
    ...
    public static void getMessagesArgs(Player player) {
        UtilityFunctions.executeSound(Objects.requireNonNull(plugin.getConfigYaml().getString("ADMIN-CONFIG.SOUNDS.CLICK_COMMAND_HELP")), player);
    ...
    public static void getHelpCommandGame(Player sender) {
        UtilityFunctions.executeSound(Objects.requireNonNull(plugin.getConfigYaml().getString("ADMIN-CONFIG.SOUNDS.MESSAGE_CLICK_HELP_COMMANDS")), sender);
    ...
    public static void getJump(Player player) {
        double boost = plugin.getConfigYaml().getInt("ON_VOID_TP.SETTINGS.JUMP.BOOST");
    ...
    public static void isNull(Player player) {
        var data = plugin.getPlayerManager().getPlayer(player.getUniqueId());

        if (!data.isTeleportEnabled() &&
                !data.isJumpEnabled() &&
                !data.isParticlesEnabled() &&
                !data.isLightningEnabled()) {
    ...
    public static void createWorldsDeluxe(Player player, String name, String type) {
        runTask(() -> {
            if (Bukkit.getWorld(name) == null) {
                ...
                List<String> worlds = config.getStringList("ON_VOID_TP.DO_NOT_TOUCH_WORLDS");
                worlds.add(setLocation(name, type));
                config.set("ON_VOID_TP.DO_NOT_TOUCH_WORLDS", worlds);

                String path = "WORLDS." + name;
                generated.set(path + ".ENVIROMENT", type);
                generated.set(path + ".WORLD-TYPE", "FLAT");
                generated.set(path + ".DIFFICULTY", "PEACEFUL");
                ...
                generated.set(path + ".SPAWN", setLocation(name));

                try {
                    config.save();
                    generated.save();
                } catch (IOException e) {
                    plugin.getLogger().severe("The settings could not be saved: " + e.getMessage());
                }

                createEmptyWorld(name, type, "FLAT", "PEACEFUL", true, true, false, false, Integer.MAX_VALUE, false, 300);
    ...
    public static String getTPS() {
        ...
        try {
            ...
            if (minecraftVersion() >= 17) {
                ...
                serverInstance = clazz.getMethod("getServer").invoke(null);
                tpsField = clazz.getField("recentTps");
            } else {
            ...
            final double n = ((double[]) tpsField.get(serverInstance))[0];
            return (n < 20.0) ? decimalFormat.format(n) : "*20.00";
    ...
    public static void sendBackGround(String type, Player player) {
        if (type.equalsIgnoreCase("ADD")) {
            ...
            runTaskLater(plugin.getConfigYaml().getInt("ON_VOID_TP.SETTINGS.BACKGROUND_EFFECT.TIME-REMOVE"), () -> player.removePotionEffect(PotionEffectType.BLINDNESS));
        }
    }
    ...
    public static boolean isMySQL() {
        return Objects.equals(plugin.getConfigYaml().getString("ADMIN-CONFIG.DATABASE.ENABLED"), "true");
    }
    ...
    public static void runTask(Runnable runnable, Location location) {
        DeluxeVoidWorld.getScheduler().runTask(location, runnable);
    }
